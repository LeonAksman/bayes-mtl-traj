function [varargout] = blr_mtl_mkl_inv_diag(hyp, X, t, nTasks, numBlocks, extraKernels, xs)
...
%*** preallocate dSigmas structure
numHypsPerBlock     = 1 + nTasks + nKernels + numInternalHypers;
dSigmas          	= zeros(nTasks*numBlocks, nTasks*numBlocks, 1 + numBlocks * numHypsPerBlock);
...
dSigmas(:, :, length(dHypers))      = kron(eye(nTasks), eye(numBlocks)); 
...
for i = 1:numBlocks
    ...
    alpha1s         = zeros(nTasks, 1);
    for j = 1:nTasks
        alpha1s(j) 	= exp_hyps_i(j);
        
        v_j         = zeros(nTasks, 1);
        v_j(j)      = 1;
        delta_j     = v_j * v_j';
          
        dHypers{end+1}                  = alpha1s(j);  
       	%dSigmas{end+1}                 = kron(delta_j,  delta_i);
        dSigmas(:, :, length(dHypers))  = kron(delta_j,  delta_i);
        
        %index_ij    = numBlocks * (j-1) + j;        
        %dSigmas(index_ij, index_ij, length(dHypers))  = 1;
    end
    
    %**** fully coupled part
    dHypers{end+1}                      = alpha2;
    dSigmas(:, :, length(dHypers))      = kron(ones(nTasks), delta_i);  
        
    %currHyp         = 3;
    %coupling_i     	= alpha1 * eye(nTasks) + (alpha2/nTasks) * ones(nTasks); 
    
    currHyp         = nTasks + 2;
    coupling_i     	= diag(alpha1s) + (alpha2/nTasks) * ones(nTasks);     
    ...
    for j = 1:nKernels
        ...
        switch type_j
            case 'linear'
                ...
                dSigmas(:, :, length(dHypers))  = kron(kernel_j,  delta_i);    
            ...
            case 'gaussian'
                ...
                dSigmas(:, :, length(dHypers))  = kron(-alpha_kernel_j * r_squared .* kernel_j, delta_i);
