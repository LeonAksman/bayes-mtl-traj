function [varargout] = blr_mtl_mkl_block_multilevel(hyp, X_design, X_mu, t, nTasks, numBlocks, extraKernels, xs)
%                                                   hyp, X,              t, nTasks, numBlocks, extraKernels, xs)
...
hyp_mu           	= hyp(); %***** not exponentiated - can be negative
mu                  = X_mu * hyp_mu;
t_hat               = t - mu;
...
switch VERSION
    case 'chol'       
        v        	= X_design * Sigma;
        chol_temp  	= chol((1/beta) * eye(N) + v*X_design');        
...
%m                   = beta*Q*t_hat;                    
m                   = beta*Q*t_hat + S*solve_chol(cholSigma, mu);                	% posterior mean        %******** multi-level version 
...
if nargin == 6
    ...
    nlZ             = -0.5*( N*log(beta) - nlZ_const - logdetSigma ...
                      - beta*(t_hat-X_design*m)'*(t_hat-X_design*m) - m'* invSigma_m - ...
                      logdetA );

    if nargout > 1    % derivatives?
        dnlZ        = zeros(size(hyp));
        b           = (eye(D) - beta*Q*X_design)*Q*t_hat;
        ...
        % noise precision
        dnlZ(1)     = -( N/(2*beta) - 0.5*(t_hat'*t_hat) + t_hat'*X_design*m + beta*t_hat'*X_design*b - 0.5*m'*XXm ...
                      - beta*b'*XXm - b'*invSigma_m -0.5*trace(Q*X_design) )*beta;
