function [varargout] = blr_mtl_mkl_flex_noLogAlphas(hyp, X, t, nTasks, nDimsPerTask, extraKernels, xs)
...
%alpha_kernel  	= exp(hyp(4:(4 + n_extra_kernels - 1)));     %******* removed exp here
...
for i = 1:n_extra_kernels        
    ...
    switch extraKernels(i).kernelWeightBound
        case 'none'
            alpha_kernel(i)	= hyp(4:(4 + i - 1));    
        case 'positive'
            alpha_kernel(i)	= exp(hyp(4:(4 + i - 1)));  
    end
