function [varargout] = blr_mtl_mkl_flex(hyp, X, t, nTasks, nDimsPerTask, extraKernels, xs)
...
alpha_kernel        = zeros(n_extra_kernels, 1);
...
for i = 1:n_extra_kernels        
    switch extraKernels(i).type
        ...
        case 'gaussian'     
            sigma_i         = exp(hyp(iHypCurr));           %***** leave this as > 0
    ...
    switch extraKernels(i).bound
        case 'none'
            alpha_kernel(i)	= hyp(4 + i - 1);    
        case 'positive'
            alpha_kernel(i)	= exp(hyp(4 + i - 1));  
    end
...
eye_dims            = eye(nDimsPerTask);

Sigma               = kron(couplingMat, eye_dims);

%cholSigma        	= chol(Sigma); %** upper triagle
cholSigma           = kron_chol(couplingMat, eye_dims);
%invCouplingMat      = inv(couplingMat);

%*********** alpha1
dSigma_dAlpha1    	= eye(nTasks);                  %********* kron(eye(nTasks),               eye(nDimsPerTask));
dHyper_alpha1      	= alpha1;

%*********** alpha2
dSigma_dAlpha2     	= (1/nTasks) * ones(nTasks);     %********* kron((1/nTasks) * ones(nTasks), eye(nDimsPerTask));
...
for i = 1:n_extra_kernels
    dSigmas{end+1} 	= kernels{i};                     %********** kron(kernels{i},               eye(nDimsPerTask));
...
for i = 1:n_extra_kernels
    switch extraKernels(i).type
        ...
        case 'gaussian'
            ...
            dSigmas{end+1} 	= -alpha_kernel(i) * r_squared .* kernels{i}; %****** kron(-alpha_kernel(i) * r_squared .* kernels{i}, eye(nDimsPerTask));
...
if nargin == 6
    ...
    if nargout > 1    % derivatives?
        ...
        for i = 1:length(dSigmas)
            ...
            if orig
                invSigma_dSigma  	=  solve_chol(cholSigma, kron(dSigma,eye_dims));
                trace_invSigma_dSigma = trace(invSigma_dSigma);
                
                %** new
                %invCoupling_dSigma  = couplingMat \ dSigma;
                %trace_invSigma_dSigma = trace(invCoupling_dSigma) * trace(eye_dims);  
                
                F               	= -solve_chol(cholSigma, invSigma_dSigma')';
                              
            else
                invCoupling_dSigma  = couplingMat \ dSigma;  %invCouplingMat * dSigma;

                %invSigma_dSigma     = kron(invCoupling_dSigma, eye_dims);
                trace_invSigma_dSigma = trace(invCoupling_dSigma) * trace(eye_dims);

                %F                   = -kron(invCouplingMat * invCoupling_dSigma', eye_dims)';
                F                    = -kron(couplingMat \ invCoupling_dSigma', eye_dims)'; 
            end
            
            c    	= -beta*S*F*SXt;
            
            dnlZ(i+1) = -( -0.5*trace_invSigma_dSigma + ...
