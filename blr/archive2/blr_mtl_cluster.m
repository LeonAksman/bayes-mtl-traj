function [varargout] = blr_mtl_cluster(hyp, X, t, nTasks, nDimsPerTask, kernels, xs)
...
U                   = (1/nTasks) * ones(nTasks);
I                   = eye(nTasks);
M                   = kernels.M;

couplingMat         = alpha1 * U + alpha2 * (M - U) + alpha3 * (I - M);
...
%*********** alpha1
dSigma_dAlpha1    	= kron(U,       eye(nDimsPerTask));
dHyper_alpha1      	= alpha1;

%*********** alpha2
dSigma_dAlpha2     	= kron(M - U,   eye(nDimsPerTask));
dHyper_alpha2      	= alpha2;               

%*********** alpha3
dSigma_dAlpha3     	= kron(I - M,   eye(nDimsPerTask));
dHyper_alpha3      	= alpha3;  

dSigmas             = {dSigma_dAlpha1,       dSigma_dAlpha2,    dSigma_dAlpha3};
dHypers             = {dHyper_alpha1,        dHyper_alpha2,     dHyper_alpha3};
