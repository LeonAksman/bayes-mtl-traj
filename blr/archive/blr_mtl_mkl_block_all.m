function [varargout] = blr_mtl_mkl_block_all(hyp, X, t, nTasks, numBlocks, extraKernels, xs)
...
for i = 1:(numBlocks + 1)
    ...
    dSigmas{end+1} 	= kron(eye(nTasks),  mat_i);
    dSigmas{end+1} 	= kron(ones(nTasks), mat_i); 
    ...
    for j = 1:nKernels
        ...
        switch type_j
            case 'linear'
                ...
                dSigmas{end+1} 	= kron(kernel_j,  mat_i);
            ...
            case 'gaussian'
                ...
                %*** internal hyper
                r_squared       = mat_j;
                dSigmas{end+1} 	= kron(-alpha_kernel_j * r_squared .* kernel_j, mat_i);
    ...
    Sigma                       = Sigma + kron(coupling_i, mat_i);
end

%*** hack
Sigma                           = Sigma + EXTRA_NOISE * kron(eye(nTasks), eye(numBlocks));
